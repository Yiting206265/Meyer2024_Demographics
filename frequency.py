...
with col1:
    alpha_gp = st.slider(r'$\alpha_{pl}$', min_value=0.0, max_value=3.0, value=1.43, step=0.01)
    A_bd_ln = st.slider('ln($A_{bd}$)', min_value=-10.0, max_value=0.0, value=-3.78, step=0.01)
    mean_bd = st.slider('$log_{10}(\mu_{bd})$', min_value=0.0, max_value=3.0, value=1.43, step=0.01)
    sigma_bd = st.slider('$log_{10}(\sigma_{bd})$', min_value=0.0, max_value=3.0, value=1.21, step=0.01)


# Giant Planet parameters in col2
with col2:
    alpha_bd = st.slider(r'$\alpha_{bd}$', min_value=-2.0, max_value=2.0, value=-0.36, step=0.01)
    A_pl_ln = st.slider('ln($A_{pl}$)', min_value=-10.0, max_value=0.0, value=-5.52, step=0.01)
    mu_pl = st.slider('$log_{10}(\mu_{pl})$', min_value=0.0, max_value=3.0, value=mu_pl_value, step=0.01)
    sigma_pl = st.slider('$log_{10}(\sigma_{pl})$', min_value=0.0, max_value=3.0, value=sigma_pl_value, step=0.01)
...
host_mass = st.number_input("Host Mass ($\mathrm{M_{\odot}}$)", 0.001, value=1.0)
...
if a_min<=10 and a_max <=10:
    ...
    f_subJ2 = A_pl* np.trapz([1.13/(a*np.log(a_max/10)) for a in a_values_m2], a_values_m2) * \
       np.trapz([d_q_i ** -alpha_gp for d_q_i in mass_ratio_values], mass_ratio_values) +A_bd* np.trapz([1.13/(a*np.log(a_max/10)) for a in a_values_m2], a_values_m2) * \
       np.trapz([d_q_i ** -alpha_bd for d_q_i in mass_ratio_values], mass_ratio_values)
    f_subJ = f_subJ1 + f_subJ2
elif a_min>10 and a_max >10:
    a_values_m = np.linspace(a_min,a_max, 500)
    f_subJ = A_pl* np.trapz([1.13/(a*np.log(a_max/10)) for a in a_values_m2], a_values_m2) * \
       np.trapz([d_q_i ** -alpha_gp for d_q_i in mass_ratio_values], mass_ratio_values) + A_bd* np.trapz([1.13/(a*np.log(a_max/10)) for a in a_values_m2], a_values_m2) * \
       np.trapz([d_q_i ** -alpha_bd for d_q_i in mass_ratio_values], mass_ratio_values)
